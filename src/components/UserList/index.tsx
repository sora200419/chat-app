...
const UserList: React.FC = () => {
  ...
  return (
    <div className='border-r overflow-hidden flex flex-col bg-white h-full'>
      ...
      {/* User list - reduce max height */}
      <div className='overflow-y-auto flex-1 contact-list-scrollable'>
        {isLoading ? (
      ...
      {/* Action buttons - reduce padding */}
      <div className='p-3 border-t flex space-x-2 action-buttons-compact'>
        <button className='flex-1 bg-primary text-white py-1.5 px-3 rounded hover:bg-primary-light transition-colors text-sm'>
          Meeting
        </button>
        <button className='flex-1 bg-white text-gray-700 border border-gray-300 py-1.5 px-3 rounded hover:bg-gray-50 transition-colors text-sm'>
          Schedule
        </button>
      </div>
    </div>
